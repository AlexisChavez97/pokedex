...
class ApiRequestTest < Minitest::Test
  ...
  def test_it_should_not_save_api_request_without_url
    subject = Models::ApiRequest.new

    refute subject.save
    assert_equal 0, Models::ApiRequest.all.count
  end

  def test_it_should_save_subject_with_url
    subject = Models::ApiRequest.new(url: "#{@base_url}#{@pokemon_index_path}")

    assert subject.save
    assert_equal 1, Models::ApiRequest.all.count
  end

  def test_it_should_not_save_subject_with_same_url
    subject_1 = Models::ApiRequest.new(url: "#{@base_url}#{@pokemon_index_path}")
    subject_2 = Models::ApiRequest.new(url: "#{@base_url}#{@pokemon_index_path}")
  ...
  def test_it_should_save_subject_with_different_url
    ...
    assert_equal 2, Models::ApiRequest.all.count
  end
  ...
  def test_cache_existing_request
    ...
    Models::ApiRequest.cache("#{@base_url}#{@pokemon_index_path}", @cache_policy) { initial_result }
